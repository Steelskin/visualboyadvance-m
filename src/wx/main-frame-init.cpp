...
public:
    ...
    void NetConnect(wxCommandEvent& ev) {
        ...
        if (!server) {
            ...
            if (!valid) {
                wxMessageBox(_("You must enter a valid host name"), _("Host name invalid"),
                             wxICON_ERROR | wxOK);
                return;
            }
        }
        ...
        if (state == LINK_NEEDS_UPDATE) {
            wxProgressDialog pdlg(title, connmsg, 100, dlg,
...
public:
    ...
    void Tool(wxCommandEvent& ev) {
        switch (ev.GetId()) {
            case wxID_OPEN: {
                wxFileDialog subdlg(
                    dlg, _("Select cheat file"), cheatdir, cheatfn,
            ...
            case wxID_ADD: {
                ...
                wxDialog* subdlg = wxGetApp().frame->GetXRCDialog("CheatEdit");
                    ...
                    if (list->GetItemState(i, wxLIST_STATE_SELECTED)) {
                        ...
                        if (isgb)
                            ...
                            if (!asked) {
                                asked = true;
                                restore =
                                    wxMessageBox(_("Restore old values?"), _("Removing cheats"),
            ...
            case wxID_CLEAR:
                if (isgb) {
                    ...
                    if (cheatsNumber) {
                        bool restore = wxMessageBox(_("Restore old values?"), _("Removing cheats"),
                                                    wxYES_NO | wxICON_QUESTION) == wxYES;
                        *dirty = true;
        ...
        if (otype != ce_type || ocode != ce_codes) {
            ...
            if (ncodes > id + 1) {
                ...
                for (int i = id + 1; i < ncodes; i++) {
                    codes[i - id - 1] = wxString(
                        isgb ? gbCheatList[i].cheatCode : cheatsList[i].codestring, wxConvLibc);
                    descs[i - id - 1] =
                        wxString(isgb ? gbCheatList[i].cheatDesc : cheatsList[i].desc, wxConvUTF8);
...
enum cf_vfmt { CFVFMT_SD, CFVFMT_UD, CFVFMT_UH };
...
public:
    ...
    void Search(wxCommandEvent& ev) {
        ...
        if (valsrc)
            ...
            cheatSearchValue(&cheatSearchData, op, size, fmt == CFVFMT_SD, SignedValue());
            ...
            for (int j = 0; j < block->size; j += (1 << size)) {
                if (IS_BIT_SET(block->bits, j)) {
                    ...
                    /*
                                                            ...
                                                            {
                                                                    wxLogError(_("Search produced %d
                       results.  Please refine better"), list->addrs.size()); list->addrs.clear();
                                                                    return;
                                                            }
    ...
    void AddCheatL(wxListEvent& ev) { AddCheat(ev.GetIndex()); }
    ...
    void AddCheat(int idx) {
        ...
        SetValVal(ca_val_tc);
        wxDialog* subdlg = wxGetApp().frame->GetXRCDialog("CheatAdd");
        dlg->SetWindowStyle(wxCAPTION | wxRESIZE_BORDER);
        ...
        if (isgb) {
            ...
            for (int i = 0; i < (1 << size); i++) {
                addr_s.Printf(wxT("%02X%02X%02X%02X"), bank, val & 0xff, addr & 0xff, addr >> 8);
    ...
    uint32_t GetValue(int fmt) { return GetValue(val_s, fmt); }

    uint32_t GetValue() { return GetValue(fmt); }
    ...
    int32_t SignedValue(int fmt) { return SignedValue(val_s, fmt); }

    int32_t SignedValue() { return SignedValue(fmt); }
    ...
    void EnableVal(wxCommandEvent& ev) { val_tc->Enable(ev.GetId() == XRCID("SpecificValue")); }
...
wxDialog* MainFrame::GetXRCDialog(const wxString& name) {
    return wxStaticCast(this->FindWindowByName(name), wxDialog);
}

wxDialog* MainFrame::LoadXRCDialog(const wxString& name) {
...
T* GetValidatedChild(wxWindow* parent, const wxString& name, V validator) {
...
void MainFrame::MenuOptionBool(const wxString& menuName, bool field) {
    const int id = wxXmlResource::GetXRCID(menuName);
    const auto iter = checkable_mi.find(id);
    if (iter == checkable_mi.end()) {
        return;
    }

    iter->second.initialized = true;
    iter->second.mi->Check(field);
}
...
void MainFrame::MenuOptionIntMask(const wxString& menuName, int field, int mask) {
    ...
    iter->second.initialized = true;
    iter->second.mask = mask;
    iter->second.val = value;
    iter->second.mi->Check((field & mask) == value);
}
...
void MainFrame::MenuOptionIntRadioValue(const wxString& menuName, int field, int value) {
    ...
    iter->second.initialized = true;
    iter->second.val = field;
    iter->second.mi->Check(field == value);
}
...
void MainFrame::GetMenuOptionBool(const wxString& menuName, bool* field) {
    ...
    *field = iter->second.mi->IsChecked();
}

void MainFrame::GetMenuOptionConfig(const wxString& menu_name, const config::OptionID& option_id) {
    config::Option* option = config::Option::ByID(option_id);
    assert(option);

    const int id = wxXmlResource::GetXRCID(menu_name);
    const auto iter = checkable_mi.find(id);
    if (iter == checkable_mi.end()) {
        return;
    }

    const bool is_checked = iter->second.mi->IsChecked();
    switch (option->type()) {
        case config::Option::Type::kBool:
            option->SetBool(is_checked);
            break;
        case config::Option::Type::kInt:
            option->SetInt(is_checked ? 1 : 0);
            break;
        default:
            assert(false);
            return;
...
void MainFrame::GetMenuOptionInt(const wxString& menuName, int* field, int mask) {
    ...
    is_checked = iter->second.mi->IsChecked();
...
void MainFrame::SetMenuOption(const wxString& menuName, bool value) {
    ...
    iter->second.mi->Check(value);
}

void MainFrame::BindAppIcon() {
    wxIcon icon = wxXmlResource::Get()->LoadIcon("MainIcon");
    if (!icon.IsOk()) {
...
bool MainFrame::BindControls() {
    ...
    Bind(wxEVT_MENU, [this](wxCommandEvent& event) {
        ExecuteHandler(config::XRCIDToHandlerID(event.GetId()));
        event.Skip(false);
    });

#if 0
    // doesn't work in 2.9 at all (causes main menu to malfunction)
    ...
    recent_menu_ = menubar->GetRecentMenu();
    OnHistoryChanged(config_provider_->file_history());

    for (auto pair : menubar->item_map()) {
        widgets::ShortcutMenuItem* menu_item = pair.second;
        ...
        if (menu_item->GetItemLabel().empty()) {
            menu_item->SetItemLabel(wxGetStockLabel(menu_item->GetId(), wxSTOCK_WITH_MNEMONIC));
        }

        // Store checkable items.
        if (menu_item->IsCheckable()) {
            checkable_mi_t cmi = {menu_item->GetId(), menu_item, 0, 0};
            checkable_mi.insert(std::make_pair(menu_item->GetId(), cmi));
        }
    }
    ...
    if (checkable_mi.size()) {
        ...
        MenuOptionBool("LoadStateAutoLoad", OPTION(kGenAutoLoadLastState));
        MenuOptionIntMask("LoadStateKeepSaves", coreOptions.skipSaveGameBattery, 1);
        MenuOptionIntMask("LoadStateKeepCheats", coreOptions.skipSaveGameCheats, 1);

        // Emulation
        MenuOptionBool("Pause", paused);
        MenuOptionBool("EmulatorSpeedupToggle", turbo);
        MenuOptionBool("VSync", OPTION(kPrefVsync));
        MenuOptionBool("FrameSkipAuto", OPTION(kPrefAutoFrameSkip));
        MenuOptionBool("SkipIntro", OPTION(kPrefSkipBios));
        MenuOptionBool("ApplyPatches", OPTION(kPrefAutoPatch));
        MenuOptionBool("PauseWhenInactive", OPTION(kPrefPauseWhenInactive));

        // Options - Link
        MenuOptionIntRadioValue("LinkTypeNothing", gopts.gba_link_type, 0);
        MenuOptionIntRadioValue("LinkTypeCable", gopts.gba_link_type, 1);
        MenuOptionIntRadioValue("LinkTypeWireless", gopts.gba_link_type, 2);
        MenuOptionIntRadioValue("LinkTypeGameCube", gopts.gba_link_type, 3);
        MenuOptionIntRadioValue("LinkTypeGameboy", gopts.gba_link_type, 4);
        MenuOptionBool("LinkLocal", OPTION(kGBALinkProto));
        MenuOptionBool("LinkAuto", OPTION(kGBALinkAuto));
        MenuOptionBool("LinkSpeedOn", OPTION(kGBALinkFast));

        // Options - Video
        MenuOptionBool("Fullscreen", OPTION(kGeomFullScreen));
        MenuOptionBool("RetainAspect", OPTION(kDispStretch));
        MenuOptionBool("Bilinear", OPTION(kDispBilinear));
#if 0
        // This is currently broken.
        MenuOptionBool("Multithread", OPTION(kTODO));
#endif
        MenuOptionBool("KeepOnTop", OPTION(kDispKeepOnTop));
        MenuOptionBool("NoStatusMsg", OPTION(kPrefDisableStatus));
        MenuOptionBool("Transparent", OPTION(kPrefShowSpeedTransparent));

        // Options - Audio
        MenuOptionBool("GBASoundInterpolation", OPTION(kSoundGBAInterpolation));
        MenuOptionBool("GBEnhanceSound", OPTION(kSoundGBEnableEffects));
        MenuOptionBool("GBSurround", OPTION(kSoundGBSurround));
        MenuOptionBool("GBDeclicking", OPTION(kSoundGBDeclicking));

        // Options - Input
        MenuOptionBool("AllowKeyboardBackgroundInput", OPTION(kUIAllowKeyboardBackgroundInput));
        MenuOptionBool("AllowJoystickBackgroundInput", OPTION(kUIAllowJoystickBackgroundInput));
        ...
        MenuOptionBool("RTC", OPTION(kPrefRTCEnabled));
        MenuOptionBool("BootRomGba", OPTION(kUIAllowJoystickBackgroundInput));
        MenuOptionBool("AGBPrinter", OPTION(kPrefUseBiosGBA));
        MenuOptionBool("GBALcdFilter", OPTION(kGBALCDFilter));

        // Options - Game Boy
        MenuOptionBool("GBColorOption", OPTION(kGBColorOption));
        MenuOptionBool("GBLcdFilter", OPTION(kGBLCDFilter));
        MenuOptionBool("ColorizerHack", OPTION(kGBColorizerHack));
        MenuOptionBool("Printer", OPTION(kPrefGBPrinter));
        MenuOptionBool("PrintGather", OPTION(kGBPrintAutoPage));
        MenuOptionBool("PrintSnap", OPTION(kGBPrintScreenCap));
        MenuOptionBool("BootRomGB", OPTION(kPrefUseBiosGB));
        MenuOptionBool("BootRomGBC", OPTION(kPrefUseBiosGBC));

        // Options - UI Settings
        MenuOptionBool("StatusBar", OPTION(kGenStatusBar));
        MenuOptionBool("HideMenuBar", OPTION(kUIHideMenuBar));
        MenuOptionBool("SuspendScreenSaver", OPTION(kUISuspendScreenSaver));

        // Tools - Cheats
        MenuOptionBool("CheatsAutoSaveLoad", OPTION(kPrefAutoSaveLoadCheatList));
        MenuOptionBool("CheatsEnable", OPTION(kPrefCheatsEnabled));

        // Tools - GDB
        MenuOptionBool("DebugGDBBreakOnLoad", OPTION(kPrefGDBBreakOnLoad));
    ...
    for (const auto pair : checkable_mi) {
        if (!pair.second.initialized) {
            wxLogError(_("Invalid menu item %s; removing"),
                       pair.second.mi->GetItemLabelText().c_str());
            pair.second.mi->GetMenu()->Remove(pair.second.mi);
        }
    }
        ...
        // just verify preview window & mag sel present
        {
            ...
            if (!wxDynamicCast(prev->GetParent(), wxScrolledWindow))
                throw std::runtime_error(
                    "Unable to load a dialog control from the builtin xrc file: Preview");
...
#define getrbo(name, option_id, value)                                          \
    do {                                                                        \
        ...
        rb->SetValidator(::widgets::OptionSelectedValidator(option_id, value)); \
    } while (0)
#define getrbi(n, o, v)                              \
    do {                                             \
...
#define getutc(n, o)                           \
    do {                                       \
...
#ifndef NO_LINK
        {
            ...
            if (okb)  // may be gone if style guidlines removed it
            {
                ...
                d->Connect(XRCID("Server"), wxEVT_COMMAND_RADIOBUTTON_SELECTED,
                           wxCommandEventHandler(NetLink_t::ServerOKButton), NULL,
                           &net_link_handler);
                d->Connect(XRCID("Client"), wxEVT_COMMAND_RADIOBUTTON_SELECTED,
                           wxCommandEventHandler(NetLink_t::ClientOKButton), NULL,
                           &net_link_handler);
            }

            // Bind server IP when the server radio button is selected.
            d->Connect(XRCID("Server"), wxEVT_COMMAND_RADIOBUTTON_SELECTED,
                       wxCommandEventHandler(NetLink_t::BindServerIP), NULL, &net_link_handler);
            // Bind client link_host when client radio button is selected.
            d->Connect(XRCID("Client"), wxEVT_COMMAND_RADIOBUTTON_SELECTED,
                       wxCommandEventHandler(NetLink_t::BindLinkHost), NULL, &net_link_handler);

            // this should intercept wxID_OK before the dialog handler gets it
            d->Connect(wxID_OK, wxEVT_COMMAND_BUTTON_CLICKED,
                       wxCommandEventHandler(NetLink_t::NetConnect), NULL, &net_link_handler);
            d->Fit();
        }
#endif
        d = LoadXRCDialog("CheatList");
        {
            ...
            if (!cl->Init())
                throw std::runtime_error(
                    "Unable to initialize the Cheats dialog control from the builtin xrc file!");
            ...
            wxFont f(of.GetPointSize(), wxFONTFAMILY_MODERN, of.GetStyle(), of.GetWeight());
...
#endif
            d->Connect(wxEVT_COMMAND_TOOL_CLICKED, wxCommandEventHandler(CheatList_t::Tool), NULL,
                       &cheat_list_handler);
            d->Connect(wxEVT_COMMAND_LIST_ITEM_CHECKED, wxListEventHandler(CheatList_t::Check),
                       NULL, &cheat_list_handler);
            d->Connect(wxEVT_COMMAND_LIST_ITEM_UNCHECKED, wxListEventHandler(CheatList_t::UnCheck),
                       NULL, &cheat_list_handler);
            d->Connect(wxEVT_COMMAND_LIST_ITEM_ACTIVATED, wxListEventHandler(CheatList_t::Edit),
                       NULL, &cheat_list_handler);
            d->Fit();
        }
        ...
        wxChoice* ch;
        {
            ...
            ch = GetValidatedChild<wxChoice, wxGenericValidator>(
                d, "Type", wxGenericValidator(&cheat_list_handler.ce_type));
...
#define cf_make_update()                            \
    rb->Connect(wxEVT_COMMAND_RADIOBUTTON_SELECTED, \
                wxCommandEventHandler(CheatFind_t::UpdateView), NULL, &cheat_find_handler)
...
#define cf_make_valen()                                                                            \
    rb->Connect(wxEVT_COMMAND_RADIOBUTTON_SELECTED, wxCommandEventHandler(CheatFind_t::EnableVal), \
...
#define cf_button(n, f)                                                                       \
    d->Connect(XRCID(n), wxEVT_COMMAND_BUTTON_CLICKED, wxCommandEventHandler(CheatFind_t::f), \
               NULL, &cheat_find_handler);
    ...
    do {                                                    \
        ...
        cheat_find_handler.v->Disable();                    \
            ...
            d->Connect(wxEVT_COMMAND_LIST_ITEM_ACTIVATED,
                       wxListEventHandler(CheatFind_t::AddCheatL), NULL, &cheat_find_handler);
            d->Connect(wxEVT_COMMAND_LIST_ITEM_SELECTED, wxListEventHandler(CheatFind_t::Select),
                       NULL, &cheat_find_handler);
            d->Fit();
        }
    ...
    do {                                       \
    ...
    } while (0)
        {
            ...
            throttle_ctrl.thr->Connect(wxEVT_COMMAND_SPINCTRL_UPDATED,
                                       wxSpinEventHandler(ThrottleCtrl_t::SetThrottleSel), NULL,
                                       &throttle_ctrl);
            throttle_ctrl.thrsel->Connect(wxEVT_COMMAND_CHOICE_SELECTED,
                                          wxCommandEventHandler(ThrottleCtrl_t::SetThrottle), NULL,
                                          &throttle_ctrl);
            d->Connect(wxEVT_SHOW, wxShowEventHandler(ThrottleCtrl_t::Init), NULL, &throttle_ctrl);
            d->Fit();
        }
        ...
        {
            speedup_throttle_ctrl.frame_skip_cb =
                SafeXRCCTRL<wxCheckBox>(d, "SpeedupThrottleFrameSkip");
            speedup_throttle_ctrl.speedup_throttle_spin =
                SafeXRCCTRL<wxSpinCtrl>(d, "SpeedupThrottleSpin");

            speedup_throttle_ctrl.speedup_throttle_spin->Connect(
                wxEVT_SPIN_UP, wxCommandEventHandler(SpeedupThrottleCtrl_t::SetSpeedupThrottle),
                NULL, &speedup_throttle_ctrl);

            speedup_throttle_ctrl.speedup_throttle_spin->Connect(
                wxEVT_SPIN_DOWN, wxCommandEventHandler(SpeedupThrottleCtrl_t::SetSpeedupThrottle),
                NULL, &speedup_throttle_ctrl);

            speedup_throttle_ctrl.speedup_throttle_spin->Connect(
                wxEVT_SPIN, wxCommandEventHandler(SpeedupThrottleCtrl_t::SetSpeedupThrottle), NULL,
                &speedup_throttle_ctrl);

            speedup_throttle_ctrl.speedup_throttle_spin->Connect(
                wxEVT_TEXT, wxCommandEventHandler(SpeedupThrottleCtrl_t::SetSpeedupThrottle), NULL,
                &speedup_throttle_ctrl);

            speedup_throttle_ctrl.frame_skip_cb->Connect(
                wxEVT_CHECKBOX, wxCommandEventHandler(SpeedupThrottleCtrl_t::SetSpeedupFrameSkip),
                NULL, &speedup_throttle_ctrl);

            d->Connect(wxEVT_SHOW, wxShowEventHandler(SpeedupThrottleCtrl_t::Init), NULL,
                       &speedup_throttle_ctrl);

            d->Fit();
        }

        wxMenuItem* suspend_scr_saver_mi = menubar->FindItem(XRCID("SuspendScreenSaver"), 0);
...
#define getfp(n, o, l)                                     \
    do {                                                   \
        ...
        d = LoadXRCDialog("GameBoyAdvanceConfig");
        {
            ...
            ch = GetValidatedChild<wxChoice, wxGenericValidator>(
                d, "SaveType", wxGenericValidator(&coreOptions.cpuSaveType));
            BatConfigHandler.type = ch;
            ch = GetValidatedChild<wxChoice, widgets::OptionChoiceValidator>(
                d, "FlashSize", widgets::OptionChoiceValidator(config::OptionID::kPrefFlashSize));
            BatConfigHandler.size = ch;
            d->Connect(XRCID("SaveType"), wxEVT_COMMAND_CHOICE_SELECTED,
                       wxCommandEventHandler(BatConfig_t::ChangeType), NULL, &BatConfigHandler);
#define getgbaw(n)                             \
    do {                                       \
        ...
        w->SetValidator(GBACtrlEnabler());     \
            ...
            d->Connect(XRCID("Detect"), wxEVT_COMMAND_BUTTON_CLICKED,
                       wxCommandEventHandler(BatConfig_t::Detect), NULL, &BatConfigHandler);
            /// Boot ROM
        ...
        dialogs::JoypadConfig::NewInstance(this, config_provider_);
...
#endif
        if (menubar) {
            dialogs::AccelConfig::NewInstance(this, menubar, config_provider_);
        }
